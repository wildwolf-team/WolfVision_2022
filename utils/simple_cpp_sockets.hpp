...
inline Socket::Socket(const SocketType socket_type) : m_socket(), m_addr()
{
#ifdef WIN32
...
inline void Socket::set_port(u_short port)
{
...
#else
inline Socket::~Socket() = default;
#endif


inline UDPClient::UDPClient(u_short port, const std::string& ip_address) : Socket(SocketType::TYPE_DGRAM)
{
...
inline ssize_t UDPClient::send_message(const std::string& message)
{
    size_t message_length = message.length();
    return sendto(m_socket, message.c_str(), message_length, 0, reinterpret_cast<sockaddr*>(&m_addr), sizeof(m_addr));
};

inline UDPServer::UDPServer(u_short port, const std::string& ip_address) : Socket(SocketType::TYPE_DGRAM)
{
    set_port(port);
    set_address(ip_address);
...
inline int UDPServer::socket_bind()
{
...
inline TCPClient::TCPClient(u_short port, const std::string& ip_address) : Socket(SocketType::TYPE_STREAM)
{
    set_address(ip_address);
    set_port(port);
...
inline int TCPClient::make_connection()
{
...
inline int TCPClient::send_message(const std::string& message)
{
...
inline TCPServer::TCPServer(u_short port, const std::string& ip_address) : Socket(SocketType::TYPE_STREAM)
{
    set_port(port);
    set_address(ip_address);
...
inline int TCPServer::socket_bind()
{
    if (bind(m_socket, reinterpret_cast<sockaddr*>(&m_addr), sizeof(m_addr)) == SOCKET_ERROR)
    {
